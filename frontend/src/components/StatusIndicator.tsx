...
export function StatusIndicator({ state, durationSecs }: Readonly<Props>) {
    ...
    const isApproachingLimit =
        isRecording && durationSecs >= WARNING_THRESHOLD_SECS
    ...
    return (
        <div className="flex items-center gap-2">
            ...
            <span
                className={`text-xs font-medium ${isOverLimit ? 'text-amber-400' : isApproachingLimit ? 'text-amber-400/80' : 'text-white/60'}`}
            >
                {getStatusText()}
                {isRecording && (
                    <span
                        className={`ml-1 font-mono ${isOverLimit ? 'text-amber-400/70' : isApproachingLimit ? 'text-amber-400/60' : 'text-white/40'}`}
                    >
